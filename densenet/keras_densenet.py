...
from densenet.custom_layer import Scale
...
def dense_block(x, stage, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1e-4,
    ...
    for i in range(nb_layers):
        ...
        concat_feat = concatenate([concat_feat, x], mode='concat', axis=concat_axis,
                                  name='concat_' + str(stage) + '_' + str(branch))
...
def test():
    ...
    train_img, train_label = load_resized_training_data(224, 224)
    test_img, test_label = load_resized_validation_data(224, 224)
    ...
    hist = model.fit(train_img, train_label, batch_size=batch_size, epochs=num_epoch, verbose=1,
                     shuffle=True, validation_data=(test_img, test_label), callbacks=[tensorboard])
